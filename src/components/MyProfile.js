...
const MyProfile = ({ token, username }) => {
  ...
  useEffect(updateProfile, [token, username, isUpdating, avatar])

  function updateProfile () {
    getUserProfile(token, username).then(profile => setUserProfile(profile))
        ...
        <div className='member-dashboard-container'>
          ...
          {(!isUpdating)
            ? <div className='member-dashboard-container'>
              ...
              {myProfile.chores.map((chore, idx) => (
