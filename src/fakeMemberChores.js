export const fakeMemberChores = [
  {
    username: 'jessicarabbit',
    chore_name: 'laundry',
    days: [{ day: 'monday', status: true }, { day: 'tuesday', status: false }, { day: 'wednesday', status: true }, { day: 'thursday', status: false }, { day: 'friday', status: true },
      { day: 'saturday', status: true }, { day: 'sunday', status: false }, { day: 'anyday', status: false }]
  },
  {
    username: 'elmerfudd',
    chore_name: 'laundry',
    days: [{ day: 'monday', status: false }, { day: 'tuesday', status: true }, { day: 'wednesday', status: true }, { day: 'thursday', status: false }, { day: 'friday', status: true },
      { day: 'saturday', status: true }, { day: 'sunday', status: false }, { day: 'anyday', status: false }]
  },
  {
    username: 'jessicarabbit',
    chore_name: 'brush teeth',
    days: [{ day: 'monday', status: false }, { day: 'tuesday', status: false }, { day: 'wednesday', status: true }, { day: 'thursday', status: false }, { day: 'friday', status: true },
      { day: 'saturday', status: false }, { day: 'sunday', status: false }, { day: 'anyday', status: false }]
  },
  {
    username: 'jessicarabbit',
    chore_name: 'dishes',
    days: [{ day: 'monday', status: true }, { day: 'tuesday', status: true }, { day: 'wednesday', status: true }, { day: 'thursday', status: false }, { day: 'friday', status: true },
      { day: 'saturday', status: true }, { day: 'sunday', status: true }, { day: 'anyday', status: false }]
  },
  {
    username: 'elmerfudd',
    chore_name: 'mow lawn',
    days: [{ day: 'monday', status: true }, { day: 'tuesday', status: true }, { day: 'wednesday', status: true }, { day: 'thursday', status: false }, { day: 'friday', status: true },
      { day: 'saturday', status: true }, { day: 'sunday', status: false }, { day: 'anyday', status: false }]
  },
  {
    username: 'elmerfudd',
    chore_name: 'vacuum',
    days: [{ day: 'monday', status: false }, { day: 'tuesday', status: false }, { day: 'wednesday', status: false }, { day: 'thursday', status: false }, { day: 'friday', status: false },
      { day: 'saturday', status: false }, { day: 'sunday', status: false }, { day: 'anyday', status: true }]
  }
]

export default fakeMemberChores
